document.addEventListener('DOMContentLoaded', function() {
    (function() {
        ...
        for (let i = 0; i < topCanciones.length; i++) {
            ...
            let popularidad = cancion.popularity
            let trackId = cancion.id
            datos.push(trackId)
            ...
            nombreCancionP.innerText = contador+".- "+nombreCancion;

            nombreCancionP.classList.add('text-sm', 'text-white', 'mb-2', 'hover:text-blue-700');
            nombreCancionP.addEventListener('click', function() {
                window.open(cancion.external_urls.spotify)
            });
            nombresDiv.appendChild(nombreCancionP)

            let graficoEspacio = document.createElement('div')
            graficoEspacio.id = `miGrafico-${i}`
            let analizador = new Analizador(trackId, access_token, graficoEspacio)
            analizador.obtenerDatos()

            let cancionImp = document.createElement('div');
            cancionImp.innerHTML = `

            <tr class=" flex items-center justify-center space-x-8 max-w-sm rounded overflow-hidden shadow-lg">
            <div class="flex items-center justify-center space-x-8 max-w-sm rounded overflow-hidden shadow-lg border rounded-l-lg rounded-lg bg-gradient-to-r from-green-700 from-10% via-green-600 via-40% to-green-400 to-90%">
            <th class="flex-shrink-0 w-48 h-48 bg-gray-400 rounded overflow-hidden mr-4">
            ...
            <div class="space-y-4 text-center mr-4">
                ...
                <p>Popularidad entre la comunidad de ${popularidad}%</p>
                <div class="flex items-center justify-center">
                ${cancion.preview_url ? `<audio src="${cancion.preview_url}" controls class="text-center justify-center items-center"></audio>` : ''}
                </div>
                <br>
            </div>
            </th>
            ...
            <th>
                <img src="../spotify-icons-logos/spotify-icons-logos/icons/02_CMYK/02_PNG/Spotify_Icon_CMYK_Black.png" alt="logo" width="40px" >
            </th>
            
            <th class="flex items-center justify-center" id="miGrafico-${i}"></th>
            </div>
            
            </tr>
            <br>`
            cancionImp.appendChild(graficoEspacio)
    ...
    <div class="flex items-center justify-center h-full flex-col">
      ...
      <img src="${topCanciones[0].album.images[0].url}" alt="Artista Album" class="flex items-center justify-center h-full rounded-l-lg rounded-r-md" width="160px">
        ...
        for (let i = 0; i < topCanciones.length; i++) {
            let cancionAgregar = topCanciones[i]
            let uri = cancionAgregar.uri
            cancionUri.push(uri)
        }

        async function createPlaylist(tracksUri) {
            let {id: user_id} = await fetchWebApi('v1/me', 'GET')

            let playlist = await fetchWebApi(
